...
{
    $sql = 'SELECT * , nickname FROM posts INNER JOIN authors ON posts.authors_id = authors.id 
...
{
    $sql = 'SELECT title, text, first_date, nickname FROM posts INNER JOIN authors ON posts.authors_id = authors.id 
            WHERE posts.id = :id';
    $sth = $isDB->prepare($sql);
...
{
    $sql = 'SELECT text, nickname FROM comments INNER JOIN authors ON comments.authors_id = authors.id 
            WHERE posts_id = :id';
    $sth = $isDB->prepare($sql);
    $sth->bindValue(":id", $idPost, PDO::PARAM_INT);
    $sth->execute();
