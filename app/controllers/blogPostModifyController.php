<?php
    ...
    if(isset($_POST['submit'])) {
        if(empty($modifyPostTitle) || empty($modifyPostText) || empty($modifyPostFirstDate) || empty($modifyPostEndDate) || empty($modifyPostImportance) || empty($modifyPostAuthor)) {
            $isNotValid = true;
            echo "Modification d'article non terminée : veuillez respecter les champs.";
        }
        if($isNotValid === false) {
            blogPostUpdate($db, $id, $modifyPostTitle, $modifyPostText, $modifyPostFirstDate, $modifyPostEndDate, $modifyPostImportance, $modifyPostAuthor);
        }
    }
